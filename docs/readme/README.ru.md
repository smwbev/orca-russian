...
<p align="center">
  <a href="https://github.com/smwbev/orca-russian/releases"><img src="https://img.shields.io/badge/версия-5.1.5-08C?style=flat" alt="Версия пакета 5.1.5" /></a>
  <img src="https://img.shields.io/badge/покрытие-98.2%25-08C?style=flat" alt="Переведено 98,2 % каталога интерфейса" />
  <img src="https://img.shields.io/badge/строк-11%20785-08C?style=flat" alt="11 785 переведённых строк" />
...
<p align="center">
  ...
  11 785 строк — меню, настройки, терминал, редактор, GitHub, GitLab, Linear, Jira, мобильный режим и онбординг.
</p>
...
> Названия внутри раздела плагинов приведены по-английски намеренно: этот раздел Orca пока запрещает переводить, поэтому он остаётся английским даже после установки пакета. Запрет сузится, когда выйдет сборка с [нашим PR #12455](https://github.com/stablyai/orca/pull/12455). Подробнее — [ниже](#почему-часть-строк-остаётся-английской).
...
217 строк перевести невозможно на нынешних сборках Orca — это ограничение движка, а не пробел в работе:

- **214 строк** — раздел настроек плагинов. Движок защищает пространство имён `auto.components.settings.plugin*` (проверка регистронезависимая). Если языковой пакет переопределит хотя бы один такой ключ, Orca отклонит **весь пакет целиком**, и русский язык просто не загрузится.
...
- **ещё 34** ждут релиза. [PR #12455](https://github.com/stablyai/orca/pull/12455) сузил правило защиты: заголовки разделов, пустые состояния, «Обновить» и форма локальной разработки не несут решения о доверии и стали переводимыми. Он влит в `main`, но ещё не вошёл ни в одну опубликованную сборку, поэтому пакет эти ключи не включает: с ними любая нынешняя сборка отвергла бы пакет целиком. Они вернутся, как только выйдет релиз с этой правкой.
...
locales/ru-RU.json      каталог переводов, 11 785 строк
...
|---|---|---|
| Узлов в каталоге | 20 000 | 13 200 |
